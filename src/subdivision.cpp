 ...
 private:
  ...
  static Partition GetCachedPartition(ivec4 n) {
    ...
    if (n[3] > 0) {  // quad
      ...
      for (const int i : {0, 1, 2, 3}) {
        ...
        for (int j = 1; j < n[i]; ++j) {
          partition.vertBary.push_back(
              la::lerp(partition.vertBary[i], nextBary, (scalar)j / n[i]));
        }
      }
      ...
      const scalar f = n[2] * n[2] + n[0] * n[0];
      if (n[1] == 1) {
        if (n[0] == 1) {
        ...
        for (int j = 1; j < nh; ++j) {
          partition.vertBary.push_back(
              la::lerp(partition.vertBary[2], middleBary, (scalar)j / nh));
        }
  ...
  static void PartitionQuad(Vec<ivec3>& triVert, Vec<vec4>& vertBary,
    ...
    for (int i = 1; i < partitions; ++i) {
      ...
      const int added = std::round(la::lerp(
          (scalar)edgeAdded[0], (scalar)edgeAdded[2], (scalar)i / partitions));
...
Vec<Barycentric> Manifold::Impl::Subdivide(
  ...
  if (numProp_ > 0) {
    ...
    Vec<scalar> prop(numProp_ * (numPropVert + addedVerts + totalEdgeAdded));
